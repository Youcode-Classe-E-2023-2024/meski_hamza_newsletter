<button class="border-b-[1px] border-l-[1px] border-r-[1px] border-solid border-gray-300 text-white px-4 py-2 h-10 w-20 rounded-br-md rounded-bl-md" onclick="openModal('modelConfirm{{$template->id}}')">
...
<div id="modelConfirm{{$template->id}}" class="fixed hidden z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 ">
    <div class=" relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
        ...
        <div class="p-6 pt-0 text-center text-black">
            <form action="{{ route('templates.send', $template->id) }}" method="POST" class="container mx-auto p-6">
                @csrf
                <h3 class="text-2xl font-bold mb-4">{{$template->id}} Select Users that will receive newsletters</h3>
